// Full ISO 3166-1 country list (English short names), sorted by name. The form
// stores and sends the country *name* (the rest of the pipeline — Pipedrive, DB,
// emails — uses the string), so names here are the source of truth. Alpha-2
// codes let us map an IP-geo header (Vercel x-vercel-ip-country) back to a name.
...
/** Map an alpha-2 code (e.g. from an IP-geo header) to a country name, or null. */
export function countryNameFromCode(code: string | null | undefined): string | null {
  if (!code) return null;
  return NAME_BY_CODE.get(code.trim().toUpperCase()) ?? null;
}

/** Whether a stored value matches one of our canonical country names. */
